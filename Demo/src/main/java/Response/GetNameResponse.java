/**
 * GetNameResponse.java
 *
 * This file was auto-generated from WSDL
 * by the Apache Axis 1.4 Apr 22, 2006 (06:55:48 PDT) WSDL2Java emitter.
 */

package Response;

import Client.Demo.Name;

public class GetNameResponse  implements java.io.Serializable {
    private Client.Demo.Name getNameReturn;

    public GetNameResponse() {
    }

    public GetNameResponse(
           Client.Demo.Name getNameReturn) {
           this.getNameReturn = getNameReturn;
    }


    /**
     * Gets the getNameReturn value for this GetNameResponse.
     * 
     * @return getNameReturn
     */
    public Client.Demo.Name getGetNameReturn() {
        return getNameReturn;
    }


    /**
     * Sets the getNameReturn value for this GetNameResponse.
     * 
     * @param getNameReturn
     */
    public void setGetNameReturn(Client.Demo.Name getNameReturn) {
        this.getNameReturn = getNameReturn;
    }

    private java.lang.Object __equalsCalc = null;
    public synchronized boolean equals(java.lang.Object obj) {
        if (!(obj instanceof GetNameResponse)) return false;
        GetNameResponse other = (GetNameResponse) obj;
        if (obj == null) return false;
        if (this == obj) return true;
        if (__equalsCalc != null) {
            return (__equalsCalc == obj);
        }
        __equalsCalc = obj;
        boolean _equals;
        _equals = true && 
            ((this.getNameReturn==null && other.getGetNameReturn()==null) || 
             (this.getNameReturn!=null &&
              this.getNameReturn.equals(other.getGetNameReturn())));
        __equalsCalc = null;
        return _equals;
    }

    private boolean __hashCodeCalc = false;
    public synchronized int hashCode() {
        if (__hashCodeCalc) {
            return 0;
        }
        __hashCodeCalc = true;
        int _hashCode = 1;
        if (getGetNameReturn() != null) {
            _hashCode += getGetNameReturn().hashCode();
        }
        __hashCodeCalc = false;
        return _hashCode;
    }

    // Type metadata
    private static org.apache.axis.description.TypeDesc typeDesc =
        new org.apache.axis.description.TypeDesc(GetNameResponse.class, true);

    static {
        typeDesc.setXmlType(new javax.xml.namespace.QName("http://Demo.Client", ">getNameResponse"));
        org.apache.axis.description.ElementDesc elemField = new org.apache.axis.description.ElementDesc();
        elemField.setFieldName("getNameReturn");
        elemField.setXmlName(new javax.xml.namespace.QName("http://Demo.Client", "getNameReturn"));
        elemField.setXmlType(new javax.xml.namespace.QName("http://Demo.Client", "Name"));
        elemField.setNillable(false);
        typeDesc.addFieldDesc(elemField);
    }

    /**
     * Return type metadata object
     */
    public static org.apache.axis.description.TypeDesc getTypeDesc() {
        return typeDesc;
    }

    /**
     * Get Custom Serializer
     */
    public static org.apache.axis.encoding.Serializer getSerializer(
           java.lang.String mechType, 
           java.lang.Class _javaType,  
           javax.xml.namespace.QName _xmlType) {
        return 
          new  org.apache.axis.encoding.ser.BeanSerializer(
            _javaType, _xmlType, typeDesc);
    }

    /**
     * Get Custom Deserializer
     */
    public static org.apache.axis.encoding.Deserializer getDeserializer(
           java.lang.String mechType, 
           java.lang.Class _javaType,  
           javax.xml.namespace.QName _xmlType) {
        return 
          new  org.apache.axis.encoding.ser.BeanDeserializer(
            _javaType, _xmlType, typeDesc);
    }

}
